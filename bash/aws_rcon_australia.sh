...
#load config
source /data/home/ec2-user/transnet/configs/australia-oceania.conf
...
python -m cProfile Transnet.py -p "../data/$destdir/pfile.poly" -c $continent -H $host -P $port -D $dname -U $duser -X $pass -d $destdir -V $vlevels $trans_args | tee "../logs/$destdir/transnet.log"
