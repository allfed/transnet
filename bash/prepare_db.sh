...
echo "3. filter all power nodes/ways of relations tagged with power=* OR substation=*"
osmosis --read-pbf file="$ddump" --tag-filter accept-relations power=* --tag-filter accept-ways substation=* power=* --tag-filter accept-nodes power=* --used-node --buffer --bounding-polygon file="$pfile" completeRelations=yes --write-pbf file="../data/$destdir/power_extract1.pbf"

echo "4. filter all relations tagged with route=power"
osmosis --read-pbf file="$ddump" --tag-filter accept-relations route=power route=substation --used-way --used-node --buffer --bounding-polygon file="$pfile" completeRelations=yes completeWays=yes --write-pbf file="../data/$destdir/power_extract2.pbf"

echo "5. filter all ways and its corresponding nodes tagged with power=* OR substation=*"
osmosis --read-pbf file="$ddump" --tag-filter accept-ways  substation=* power=* --used-node --buffer --bounding-polygon file="$pfile" completeWays=yes --write-pbf file="../data/$destdir/power_extract3.pbf"
